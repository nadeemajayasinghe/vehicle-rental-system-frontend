...
import { vehicleService, VehicleRequest } from '@/services/vehicleService';
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col">
        ...
        <div className="flex flex-1 flex-col items-center justify-center gap-4 text-center px-4">
          ...
          <p className="text-slate-500">You must be an admin to add vehicles.</p>
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col">
      ...
      <div className="bg-slate-950 text-white">
        <div className="mx-auto max-w-4xl px-4 py-10 sm:px-6 lg:px-8">
          ...
          <p className="mt-2 text-slate-400">Fill in the details to add a vehicle to the fleet.</p>
        </div>
      </div>
      ...
      <main className="flex-1 mx-auto w-full max-w-4xl px-4 py-10 sm:px-6 lg:px-8">
        {success && (
          <div className="mb-6 flex items-center gap-3 rounded-2xl border border-green-200 bg-green-50 px-5 py-4 text-green-700">
            ...
            <span className="font-medium">Vehicle created successfully! Redirecting...</span>
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <div className="grid gap-5 sm:grid-cols-3">
              <FormField label="Make *" name="make" value={form.make} onChange={handleChange} placeholder="e.g. Toyota" />
              <FormField label="Brand *" name="brand" value={form.brand} onChange={handleChange} placeholder="e.g. Toyota" />
              <FormField label="Model *" name="model" value={form.model} onChange={handleChange} placeholder="e.g. Camry" />
              <FormField label="Year *" name="year" type="number" value={form.year} onChange={handleChange} min={1990} max={new Date().getFullYear() + 1} />
              <FormField label="Color *" name="color" value={form.color} onChange={handleChange} placeholder="e.g. Pearl White" />
              <FormField label="Plate Number *" name="plateNumber" value={form.plateNumber} onChange={handleChange} placeholder="e.g. CAR-1234" />
            </div>
          </div>
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <div className="grid gap-5 sm:grid-cols-2">
              <FormField label="Daily Rate ($) *" name="dailyRate" type="number" value={form.dailyRate} onChange={handleChange} min={0} step={0.01} placeholder="e.g. 89.99" />
              <FormField label="Mileage (km) *" name="mileage" type="number" value={form.mileage} onChange={handleChange} min={0} placeholder="e.g. 15000" />
            </div>
          </div>
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <div className="space-y-5">
              <FormField label="Image URL" name="imageUrl" value={form.imageUrl} onChange={handleChange} placeholder="https://example.com/car.jpg" />
              {form.imageUrl && (
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={4}
                  placeholder="Describe this vehicle's features, condition, and any special notes..."
