...
export default function Navbar() {
  ...
  const navLinks = user
    ? [
      ...
      ...(user.role === 'ADMIN' ? [{ label: 'Admin', href: '/admin/vehicles' }] : []),
    ]
    : [
      { label: 'Home', href: '/' },
      { label: 'Vehicles', href: '/vehicles' },
          ...
          {navLinks.map((link) =>
            link.label === 'Admin' ? (
              <Link key={link.href} href={link.href}
                className={`rounded-full px-4 py-2 text-sm font-semibold transition ${pathname.startsWith('/admin') ? 'bg-blue-600 text-white shadow-md shadow-blue-600/25' : 'bg-blue-50 text-blue-700 hover:bg-blue-600 hover:text-white'
                  }`}>
                ⚙ Admin
              </Link>
            ) : (
              <Link key={link.href} href={link.href}
                className={`rounded-full px-4 py-2 text-sm font-medium transition ${pathname === link.href ? 'bg-slate-100 text-slate-950' : 'text-slate-600 hover:bg-slate-100 hover:text-slate-950'
                  }`}>
                {link.label}
              </Link>
            )
